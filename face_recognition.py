...
class Face_Recognition:
    def __init__(self,root):
        ...
        img=img.resize((1290,200),Image.ANTIALIAS)
        self.photoimg=ImageTk.PhotoImage(img)

        # set image as lable
        f_lb1 = Label(self.root,image=self.photoimg)
        f_lb1.place(x=0,y=0,width=1290,height=200)

        # backgorund image 
        bg1=Image.open(r"images\bg2.jpg")
        bg1=bg1.resize((1290,550),Image.ANTIALIAS)
        self.photobg1=ImageTk.PhotoImage(bg1)

        # set image as lable
        bg_img = Label(self.root,image=self.photobg1)
        bg_img.place(x=0,y=130,width=1290,height=550)

        #title section
        title_lb1 = Label(bg_img,text="Welcome to Face Recognition Pannel",font=("verdana",30,"bold"),bg="white",fg="navyblue")
        title_lb1.place(x=0,y=0,width=1290,height=60)
    ...
    def mark_attendance(self,i,r,n):
        with open("swaraj.csv","r+",newline="\n") as f:
    ...
    def face_recog(self):
        def draw_boundray(img,classifier,scaleFactor,minNeighbors,color,text,clf):
            ...
            for (x,y,w,h) in featuers:
                ...
                if confidence > 77:
                    cv2.putText(img,f"student_id:{i}",(x,y-70),cv2.FONT_HERSHEY_COMPLEX,0.8,(64,15,223),1)
                    cv2.putText(img,f"Name:{n}",(x,y-55),cv2.FONT_HERSHEY_COMPLEX,0.8,(64,15,223),1)
                    cv2.putText(img,f"Roll:{r}",(x,y-30),cv2.FONT_HERSHEY_COMPLEX,0.8,(64,15,223),1)
